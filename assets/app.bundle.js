(function() {
  ...
  */
  ...
  const addPaginationHandlers = (emblaApi, paginationNode) => {
    ...
    emblaApi.on("init", updatePagination).on("reInit", updatePagination).on("select", updatePagination);
    return () => {
  ...
  class QuantityAdjuster extends BaseComponent {
    ...
    set min(value) {
      isNumber$1(value) ? this.input.min = value : this.input.removeAttribute("min");
    }
    set max(value) {
      isNumber$1(value) ? this.input.max = value : this.input.removeAttribute("max");
    }
    set step(value) {
      isNumber$1(value) ? this.input.step = value : this.input.removeAttribute("step");
    }
    get value() {
